...
import { AlertCircle, ArrowUpRight, ChevronLeft, ChevronRight, Database, LoaderCircle, Search } from 'lucide-react'
...
export function PageHeading({ eyebrow = '', title, description, action }) { return <div className="page-heading"><div>{eyebrow && <div className="eyebrow">{eyebrow}</div>}<h1>{title}</h1>{description && <p>{description}</p>}</div>{action}</div> }
...
export function Metric({ label, value, hint }) { return <div className="metric"><div className="metric-label">{label}</div><div className="metric-value">{value}</div><div className="metric-hint">{hint}</div></div> }
